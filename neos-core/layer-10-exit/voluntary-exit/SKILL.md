...
- **Ecosystem transition**: an AZPO dissolves (per azpo-dissolution) and a member chooses not to transfer to another AZPO

## D. Required Inputs

- **Departing member identity**: confirmed ecosystem identity of the person departing
- **Departure scope**: whether the member is leaving a single AZPO or the entire ecosystem
...
6. **Confirm handoff completion.** The departure coordinator verifies that all commitments have been resolved or transferred. Any commitments that remain unresolved enter the emergency-transfer queue for the AZPO to handle post-departure. The departing member is not held responsible for commitments the ecosystem fails to reassign.
...
A Departure Record following `assets/departure-record-template.yaml`. The record contains: departure ID, member identity, departure scope (AZPO or ecosystem), declaration date, effective departure date, commitment unwinding summary (referencing the Commitment Unwinding Ledger), data export confirmation, re-entry eligibility status, and coordinator identity. The record is accessible to the departing member indefinitely and to ecosystem governance records for historical reference.
...
- **Sole critical role with no successor**: the AZPO activates emergency role coverage (Layer VIII protocols), distributing critical functions across available members until a permanent replacement is assigned through role-assignment
...
**Edge case**: During handoff, the Welcome Circle discovers that Rina is the only person who knows the login credentials for OmniOne's social media accounts. This is a sole-critical-knowledge situation. Kadek escalates to the AZPO facilitator, who arranges an emergency knowledge transfer session. The transfer is completed within the 30-day window, but the AZPO also documents the single-point-of-failure in a governance health note to prevent recurrence. Rina's departure is not delayed -- the ecosystem adapts.
...
OmniOne scales to 4,000 members across 12 SHUR locations. The voluntary-exit skill scales through AZPO-level execution: each location maintains its own departure coordinator pool and processes departures locally. The Departure Record template is identical across all locations, ensuring portable records are interoperable. At scale, the coordinator assignment algorithm draws from a wider pool, reducing the risk of social-pressure conflicts. Departure statistics are aggregated at the ecosystem level for governance health monitoring -- if one AZPO shows departure rates significantly above the mean, it triggers an automatic audit. The 30-day timeline and 7-day urgent option remain constant regardless of ecosystem size. The process does not require centralized approval at any scale.
