...
async def create_pushsubscription(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "PushSubscription created successfully.",
        },
    }



async def get_pushsubscription_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get PushSubscription by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(PushSubscription).where(PushSubscription.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(PushSubscription.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"PushSubscription not found."}
    ...
    mapper = sqlalchemy_inspect(PushSubscription)
...
async def get_pushsubscription_by_member_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get PushSubscription by business key member_id."""
    key_value = args.get("member_id", "")
    if not key_value:
        return {"success": False, "error": "member_id is required."}
    
    stmt = select(PushSubscription).where(PushSubscription.member_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(PushSubscription.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"PushSubscription with member_id='{key_value}' not found."}
...
async def list_pushsubscription(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List PushSubscription records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(PushSubscription, column.key) == args[column.key])
...
async def update_pushsubscription(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update PushSubscription by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "PushSubscription updated successfully.",
        },
    }



async def delete_pushsubscription(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete PushSubscription by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "PushSubscription deleted successfully.",
        },
    }
