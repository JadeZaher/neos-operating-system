...
async def create_agreement(
    ...
    required = ['agreement_id', 'type', 'title', 'version', 'status', 'hierarchy_level']
...
async def get_agreement_by_id(
    ...
    """Get Agreement by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Agreement).where(Agreement.id == id_uuid)
    if ecosystem_ids:
    ...
    if instance is None:
        return {"success": False, "error": f"Agreement not found."}
    ...
    mapper = sqlalchemy_inspect(Agreement)
...
async def get_agreement_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Agreement by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(Agreement).where(Agreement.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Agreement.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Agreement with ecosystem_id='{key_value}' not found."}
...
async def list_agreement(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List Agreement records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Agreement, column.key) == args[column.key])
