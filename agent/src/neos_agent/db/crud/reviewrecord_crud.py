...
from neos_agent.db.models import ReviewRecord
...
async def create_reviewrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new ReviewRecord record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ReviewRecord created successfully.",
        },
    }



async def get_reviewrecord_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ReviewRecord by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(ReviewRecord).where(ReviewRecord.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(ReviewRecord.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"ReviewRecord not found."}
    ...
    mapper = sqlalchemy_inspect(ReviewRecord)
...
async def get_reviewrecord_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ReviewRecord by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(ReviewRecord).where(ReviewRecord.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(ReviewRecord.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"ReviewRecord with ecosystem_id='{key_value}' not found."}
...
async def list_reviewrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List ReviewRecord records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(ReviewRecord, column.key) == args[column.key])
...
async def update_reviewrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update ReviewRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ReviewRecord updated successfully.",
        },
    }



async def delete_reviewrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete ReviewRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "ReviewRecord deleted successfully.",
        },
    }
