...
from neos_agent.db.models import SharesNeeds
...
async def create_sharesneeds(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "SharesNeeds created successfully.",
        },
    }



async def get_sharesneeds_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get SharesNeeds by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(SharesNeeds).where(SharesNeeds.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(SharesNeeds.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"SharesNeeds not found."}
    ...
    mapper = sqlalchemy_inspect(SharesNeeds)
...
async def get_sharesneeds_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get SharesNeeds by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(SharesNeeds).where(SharesNeeds.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(SharesNeeds.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"SharesNeeds with ecosystem_id='{key_value}' not found."}
...
async def list_sharesneeds(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List SharesNeeds records with optional filtering."""
    stmt = select(SharesNeeds)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(SharesNeeds, column.key) == args[column.key])
...
async def update_sharesneeds(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update SharesNeeds by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "SharesNeeds updated successfully.",
        },
    }



async def delete_sharesneeds(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete SharesNeeds by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "SharesNeeds deleted successfully.",
        },
    }
