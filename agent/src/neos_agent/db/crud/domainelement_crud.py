...
from neos_agent.db.models import DomainElement
...
async def create_domainelement(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new DomainElement record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "DomainElement created successfully.",
        },
    }



async def get_domainelement_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get DomainElement by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(DomainElement).where(DomainElement.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(DomainElement.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"DomainElement not found."}
    ...
    mapper = sqlalchemy_inspect(DomainElement)
...
async def get_domainelement_by_domain_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get DomainElement by business key domain_id."""
    key_value = args.get("domain_id", "")
    if not key_value:
        return {"success": False, "error": "domain_id is required."}
    
    stmt = select(DomainElement).where(DomainElement.domain_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(DomainElement.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"DomainElement with domain_id='{key_value}' not found."}
...
async def list_domainelement(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List DomainElement records with optional filtering."""
    stmt = select(DomainElement)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(DomainElement, column.key) == args[column.key])
...
async def update_domainelement(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update DomainElement by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "DomainElement updated successfully.",
        },
    }



async def delete_domainelement(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete DomainElement by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "DomainElement deleted successfully.",
        },
    }
