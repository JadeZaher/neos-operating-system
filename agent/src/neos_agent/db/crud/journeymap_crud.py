...
from neos_agent.db.models import JourneyMap
...
async def create_journeymap(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "JourneyMap created successfully.",
        },
    }



async def get_journeymap_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get JourneyMap by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(JourneyMap).where(JourneyMap.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(JourneyMap.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"JourneyMap not found."}
    ...
    mapper = sqlalchemy_inspect(JourneyMap)
...
async def get_journeymap_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get JourneyMap by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(JourneyMap).where(JourneyMap.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(JourneyMap.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"JourneyMap with ecosystem_id='{key_value}' not found."}
...
async def list_journeymap(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List JourneyMap records with optional filtering."""
    stmt = select(JourneyMap)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(JourneyMap, column.key) == args[column.key])
...
async def update_journeymap(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update JourneyMap by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "JourneyMap updated successfully.",
        },
    }



async def delete_journeymap(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete JourneyMap by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "JourneyMap deleted successfully.",
        },
    }
