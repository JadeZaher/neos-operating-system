...
from neos_agent.db.models import Message
...
async def get_message_by_id(
    ...
    """Get Message by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Message).where(Message.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(Message.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"Message not found."}
    ...
    mapper = sqlalchemy_inspect(Message)
...
async def get_message_by_conversation_id(
    ...
    """Get Message by business key conversation_id."""
    key_value = args.get("conversation_id", "")
    if not key_value:
        return {"success": False, "error": "conversation_id is required."}
    
    stmt = select(Message).where(Message.conversation_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Message.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Message with conversation_id='{key_value}' not found."}
...
async def list_message(
    ...
    """List Message records with optional filtering."""
    stmt = select(Message)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Message, column.key) == args[column.key])
