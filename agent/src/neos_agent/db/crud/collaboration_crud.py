...
from neos_agent.db.models import Collaboration
...
async def create_collaboration(
    ...
    required = ['title', 'status', 'engagement_tier']
...
async def get_collaboration_by_id(
    ...
    """Get Collaboration by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Collaboration).where(Collaboration.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(Collaboration.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"Collaboration not found."}
    ...
    mapper = sqlalchemy_inspect(Collaboration)
...
async def get_collaboration_by_source_domain_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Collaboration by business key source_domain_id."""
    key_value = args.get("source_domain_id", "")
    if not key_value:
        return {"success": False, "error": "source_domain_id is required."}
    
    stmt = select(Collaboration).where(Collaboration.source_domain_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Collaboration.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Collaboration with source_domain_id='{key_value}' not found."}
    
    # Convert to dict
    data = {
        "id": str(instance.id),
        "source_domain_id": getattr(instance, business_key),
        "ecosystem_id": str(instance.ecosystem_id),
    }
...
async def list_collaboration(
    ...
    """List Collaboration records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Collaboration, column.key) == args[column.key])
