...
from neos_agent.db.models import Domain
...
async def get_domain_by_id(
    ...
    """Get Domain by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Domain).where(Domain.id == id_uuid)
    if ecosystem_ids:
    ...
    if instance is None:
        return {"success": False, "error": f"Domain not found."}
    ...
    mapper = sqlalchemy_inspect(Domain)
...
async def get_domain_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Domain by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(Domain).where(Domain.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Domain.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Domain with ecosystem_id='{key_value}' not found."}
...
async def list_domain(
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Domain, column.key) == args[column.key])
