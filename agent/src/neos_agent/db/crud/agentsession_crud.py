...
async def create_agentsession(
    ...
    required = ['archived', 'started_at', 'status', 'privacy', 'total_tokens_used']
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AgentSession created successfully.",
        },
    }



async def get_agentsession_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AgentSession by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(AgentSession).where(AgentSession.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(AgentSession.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"AgentSession not found."}
    ...
    mapper = sqlalchemy_inspect(AgentSession)
...
async def get_agentsession_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AgentSession by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(AgentSession).where(AgentSession.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(AgentSession.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"AgentSession with ecosystem_id='{key_value}' not found."}
...
async def list_agentsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List AgentSession records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(AgentSession, column.key) == args[column.key])
...
async def update_agentsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update AgentSession by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AgentSession updated successfully.",
        },
    }



async def delete_agentsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "AgentSession deleted successfully.",
        },
    }
