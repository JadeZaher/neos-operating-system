...
from neos_agent.db.models import TestReport
...
async def create_testreport(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new TestReport record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "TestReport created successfully.",
        },
    }



async def get_testreport_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get TestReport by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(TestReport).where(TestReport.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(TestReport.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"TestReport not found."}
    ...
    mapper = sqlalchemy_inspect(TestReport)
...
async def get_testreport_by_proposal_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get TestReport by business key proposal_id."""
    key_value = args.get("proposal_id", "")
    if not key_value:
        return {"success": False, "error": "proposal_id is required."}
    
    stmt = select(TestReport).where(TestReport.proposal_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(TestReport.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"TestReport with proposal_id='{key_value}' not found."}
...
async def list_testreport(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List TestReport records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(TestReport, column.key) == args[column.key])
...
async def update_testreport(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update TestReport by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "TestReport updated successfully.",
        },
    }



async def delete_testreport(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "TestReport deleted successfully.",
        },
    }
