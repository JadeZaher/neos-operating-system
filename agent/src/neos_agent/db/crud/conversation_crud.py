...
from neos_agent.db.models import Conversation
...
async def get_conversation_by_id(
    ...
    """Get Conversation by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Conversation).where(Conversation.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(Conversation.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"Conversation not found."}
    ...
    mapper = sqlalchemy_inspect(Conversation)
...
async def get_conversation_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Conversation by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(Conversation).where(Conversation.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Conversation.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Conversation with ecosystem_id='{key_value}' not found."}
...
async def list_conversation(
    ...
    """List Conversation records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Conversation, column.key) == args[column.key])
