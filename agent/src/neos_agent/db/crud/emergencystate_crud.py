...
async def create_emergencystate(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new EmergencyState record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "EmergencyState created successfully.",
        },
    }



async def get_emergencystate_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get EmergencyState by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(EmergencyState).where(EmergencyState.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(EmergencyState.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"EmergencyState not found."}
    ...
    mapper = sqlalchemy_inspect(EmergencyState)
...
async def get_emergencystate_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get EmergencyState by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(EmergencyState).where(EmergencyState.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(EmergencyState.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"EmergencyState with ecosystem_id='{key_value}' not found."}
...
async def list_emergencystate(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List EmergencyState records with optional filtering."""
    stmt = select(EmergencyState)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(EmergencyState, column.key) == args[column.key])
...
async def update_emergencystate(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update EmergencyState by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "EmergencyState updated successfully.",
        },
    }



async def delete_emergencystate(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete EmergencyState by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "EmergencyState deleted successfully.",
        },
    }
