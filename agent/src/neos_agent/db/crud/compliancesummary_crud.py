...
from neos_agent.db.models import ComplianceSummary
...
async def create_compliancesummary(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new ComplianceSummary record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ComplianceSummary created successfully.",
        },
    }



async def get_compliancesummary_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ComplianceSummary by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(ComplianceSummary).where(ComplianceSummary.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(ComplianceSummary.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"ComplianceSummary not found."}
    ...
    mapper = sqlalchemy_inspect(ComplianceSummary)
...
async def get_compliancesummary_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ComplianceSummary by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(ComplianceSummary).where(ComplianceSummary.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(ComplianceSummary.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"ComplianceSummary with ecosystem_id='{key_value}' not found."}
...
async def list_compliancesummary(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List ComplianceSummary records with optional filtering."""
    stmt = select(ComplianceSummary)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(ComplianceSummary, column.key) == args[column.key])
...
async def update_compliancesummary(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update ComplianceSummary by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ComplianceSummary updated successfully.",
        },
    }



async def delete_compliancesummary(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete ComplianceSummary by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "ComplianceSummary deleted successfully.",
        },
    }
