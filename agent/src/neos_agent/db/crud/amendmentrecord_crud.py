...
from neos_agent.db.models import AmendmentRecord
...
async def create_amendmentrecord(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AmendmentRecord created successfully.",
        },
    }



async def get_amendmentrecord_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AmendmentRecord by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(AmendmentRecord).where(AmendmentRecord.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(AmendmentRecord.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"AmendmentRecord not found."}
    ...
    mapper = sqlalchemy_inspect(AmendmentRecord)
...
async def get_amendmentrecord_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AmendmentRecord by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(AmendmentRecord).where(AmendmentRecord.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(AmendmentRecord.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"AmendmentRecord with ecosystem_id='{key_value}' not found."}
...
async def list_amendmentrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List AmendmentRecord records with optional filtering."""
    stmt = select(AmendmentRecord)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(AmendmentRecord, column.key) == args[column.key])
...
async def update_amendmentrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update AmendmentRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AmendmentRecord updated successfully.",
        },
    }



async def delete_amendmentrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete AmendmentRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "AmendmentRecord deleted successfully.",
        },
    }
