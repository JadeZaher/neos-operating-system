...
from neos_agent.db.models import ConflictCase
...
async def create_conflictcase(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ConflictCase created successfully.",
        },
    }



async def get_conflictcase_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ConflictCase by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(ConflictCase).where(ConflictCase.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(ConflictCase.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"ConflictCase not found."}
    ...
    mapper = sqlalchemy_inspect(ConflictCase)
...
async def get_conflictcase_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ConflictCase by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(ConflictCase).where(ConflictCase.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(ConflictCase.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"ConflictCase with ecosystem_id='{key_value}' not found."}
...
async def list_conflictcase(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List ConflictCase records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(ConflictCase, column.key) == args[column.key])
...
async def update_conflictcase(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update ConflictCase by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ConflictCase updated successfully.",
        },
    }



async def delete_conflictcase(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete ConflictCase by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "ConflictCase deleted successfully.",
        },
    }
