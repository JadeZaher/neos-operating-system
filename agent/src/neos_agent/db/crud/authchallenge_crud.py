...
from neos_agent.db.models import AuthChallenge
...
async def create_authchallenge(
    args: dict,
    db: AsyncSession,
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AuthChallenge created successfully.",
        },
    }



async def get_authchallenge_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AuthChallenge by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(AuthChallenge).where(AuthChallenge.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(AuthChallenge.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"AuthChallenge not found."}
    ...
    mapper = sqlalchemy_inspect(AuthChallenge)
...
async def list_authchallenge(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List AuthChallenge records with optional filtering."""
    stmt = select(AuthChallenge)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(AuthChallenge, column.key) == args[column.key])
...
async def update_authchallenge(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update AuthChallenge by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AuthChallenge updated successfully.",
        },
    }



async def delete_authchallenge(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete AuthChallenge by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "AuthChallenge deleted successfully.",
        },
    }
