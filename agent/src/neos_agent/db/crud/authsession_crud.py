...
from neos_agent.db.models import AuthSession
...
async def create_authsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new AuthSession record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AuthSession created successfully.",
        },
    }



async def get_authsession_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AuthSession by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(AuthSession).where(AuthSession.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(AuthSession.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"AuthSession not found."}
    ...
    mapper = sqlalchemy_inspect(AuthSession)
...
async def get_authsession_by_user_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AuthSession by business key user_id."""
    key_value = args.get("user_id", "")
    if not key_value:
        return {"success": False, "error": "user_id is required."}
    
    stmt = select(AuthSession).where(AuthSession.user_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(AuthSession.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"AuthSession with user_id='{key_value}' not found."}
    
    # Convert to dict
    data = {
        "id": str(instance.id),
        "user_id": getattr(instance, business_key),
        "ecosystem_id": str(instance.ecosystem_id),
    }
...
async def list_authsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List AuthSession records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(AuthSession, column.key) == args[column.key])
...
async def update_authsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update AuthSession by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AuthSession updated successfully.",
        },
    }



async def delete_authsession(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete AuthSession by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "AuthSession deleted successfully.",
        },
    }
