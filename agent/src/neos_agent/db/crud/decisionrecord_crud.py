...
from neos_agent.db.models import DecisionRecord
...
async def create_decisionrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new DecisionRecord record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "DecisionRecord created successfully.",
        },
    }



async def get_decisionrecord_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get DecisionRecord by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(DecisionRecord).where(DecisionRecord.id == id_uuid)
    if ecosystem_ids:
    ...
    if instance is None:
        return {"success": False, "error": f"DecisionRecord not found."}
    ...
    mapper = sqlalchemy_inspect(DecisionRecord)
...
async def get_decisionrecord_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get DecisionRecord by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(DecisionRecord).where(DecisionRecord.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(DecisionRecord.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"DecisionRecord with ecosystem_id='{key_value}' not found."}
...
async def list_decisionrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List DecisionRecord records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(DecisionRecord, column.key) == args[column.key])
...
async def update_decisionrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update DecisionRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "DecisionRecord updated successfully.",
        },
    }



async def delete_decisionrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete DecisionRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "DecisionRecord deleted successfully.",
        },
    }
