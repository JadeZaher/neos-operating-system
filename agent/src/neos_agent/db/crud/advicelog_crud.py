...
from neos_agent.db.models import AdviceLog
...
async def create_advicelog(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new AdviceLog record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AdviceLog created successfully.",
        },
    }



async def get_advicelog_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AdviceLog by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(AdviceLog).where(AdviceLog.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(AdviceLog.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"AdviceLog not found."}
    ...
    mapper = sqlalchemy_inspect(AdviceLog)
...
async def get_advicelog_by_proposal_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get AdviceLog by business key proposal_id."""
    key_value = args.get("proposal_id", "")
    if not key_value:
        return {"success": False, "error": "proposal_id is required."}
    
    stmt = select(AdviceLog).where(AdviceLog.proposal_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(AdviceLog.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"AdviceLog with proposal_id='{key_value}' not found."}
...
async def list_advicelog(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List AdviceLog records with optional filtering."""
    stmt = select(AdviceLog)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(AdviceLog, column.key) == args[column.key])
...
async def update_advicelog(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update AdviceLog by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "AdviceLog updated successfully.",
        },
    }



async def delete_advicelog(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete AdviceLog by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "AdviceLog deleted successfully.",
        },
    }
