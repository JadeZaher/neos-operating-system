...
async def get_ecosystem_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Ecosystem by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Ecosystem).where(Ecosystem.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(Ecosystem.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"Ecosystem not found."}
    ...
    mapper = sqlalchemy_inspect(Ecosystem)
...
async def get_ecosystem_by_uaf_agreement_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Ecosystem by business key uaf_agreement_id."""
    key_value = args.get("uaf_agreement_id", "")
    if not key_value:
        return {"success": False, "error": "uaf_agreement_id is required."}
    
    stmt = select(Ecosystem).where(Ecosystem.uaf_agreement_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Ecosystem.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Ecosystem with uaf_agreement_id='{key_value}' not found."}
    
    # Convert to dict
    data = {
        "id": str(instance.id),
        "uaf_agreement_id": getattr(instance, business_key),
        "ecosystem_id": str(instance.ecosystem_id),
    }
...
async def list_ecosystem(
    ...
    """List Ecosystem records with optional filtering."""
    stmt = select(Ecosystem)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Ecosystem, column.key) == args[column.key])
