...
async def create_proposal(
    ...
    required = ['proposal_id', 'type', 'title', 'version', 'status']
...
async def get_proposal_by_id(
    ...
    """Get Proposal by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(Proposal).where(Proposal.id == id_uuid)
    if ecosystem_ids:
    ...
    if instance is None:
        return {"success": False, "error": f"Proposal not found."}
    ...
    mapper = sqlalchemy_inspect(Proposal)
...
async def get_proposal_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get Proposal by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(Proposal).where(Proposal.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(Proposal.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"Proposal with ecosystem_id='{key_value}' not found."}
...
async def list_proposal(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List Proposal records with optional filtering."""
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(Proposal, column.key) == args[column.key])
