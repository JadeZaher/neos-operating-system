...
from neos_agent.db.models import CircleMembership
...
async def create_circlemembership(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Create a new CircleMembership record."""
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "CircleMembership created successfully.",
        },
    }



async def get_circlemembership_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get CircleMembership by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(CircleMembership).where(CircleMembership.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(CircleMembership.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"CircleMembership not found."}
    ...
    mapper = sqlalchemy_inspect(CircleMembership)
...
async def get_circlemembership_by_domain_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get CircleMembership by business key domain_id."""
    key_value = args.get("domain_id", "")
    if not key_value:
        return {"success": False, "error": "domain_id is required."}
    
    stmt = select(CircleMembership).where(CircleMembership.domain_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(CircleMembership.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"CircleMembership with domain_id='{key_value}' not found."}
...
async def list_circlemembership(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List CircleMembership records with optional filtering."""
    stmt = select(CircleMembership)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(CircleMembership, column.key) == args[column.key])
...
async def update_circlemembership(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update CircleMembership by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "CircleMembership updated successfully.",
        },
    }



async def delete_circlemembership(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete CircleMembership by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "CircleMembership deleted successfully.",
        },
    }
