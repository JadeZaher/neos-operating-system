...
async def get_user_by_id(
    ...
    stmt = select(User).where(User.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(User.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"User not found."}
    ...
    mapper = sqlalchemy_inspect(User)
...
async def get_user_by_oauth_id(
    ...
    """Get User by business key oauth_id."""
    key_value = args.get("oauth_id", "")
    if not key_value:
        return {"success": False, "error": "oauth_id is required."}
    
    stmt = select(User).where(User.oauth_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(User.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"User with oauth_id='{key_value}' not found."}
    
    # Convert to dict
    data = {
        "id": str(instance.id),
        "oauth_id": getattr(instance, business_key),
        "ecosystem_id": str(instance.ecosystem_id),
    }
...
async def list_user(
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(User, column.key) == args[column.key])
