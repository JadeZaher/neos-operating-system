...
async def create_exitrecord(
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ExitRecord created successfully.",
        },
    }



async def get_exitrecord_by_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ExitRecord by internal UUID id."""
    id_str = args.get("id", "")
    if not id_str:
    ...
    stmt = select(ExitRecord).where(ExitRecord.id == id_uuid)
    if ecosystem_ids:
        stmt = stmt.where(ExitRecord.ecosystem_id.in_(ecosystem_ids))
    
    result = await db.execute(stmt)
    instance = result.scalars().first()
    
    if instance is None:
        return {"success": False, "error": f"ExitRecord not found."}
    ...
    mapper = sqlalchemy_inspect(ExitRecord)
...
async def get_exitrecord_by_ecosystem_id(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Get ExitRecord by business key ecosystem_id."""
    key_value = args.get("ecosystem_id", "")
    if not key_value:
        return {"success": False, "error": "ecosystem_id is required."}
    
    stmt = select(ExitRecord).where(ExitRecord.ecosystem_id == key_value)
    if ecosystem_ids:
        stmt = stmt.where(ExitRecord.ecosystem_id.in_(ecosystem_ids))
    ...
    if instance is None:
        return {"success": False, "error": f"ExitRecord with ecosystem_id='{key_value}' not found."}
...
async def list_exitrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """List ExitRecord records with optional filtering."""
    stmt = select(ExitRecord)
    ...
    for column in mapper.columns:
        if column.key in args and args[column.key]:
            stmt = stmt.where(getattr(ExitRecord, column.key) == args[column.key])
...
async def update_exitrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Update ExitRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(instance.id),
            "message": "ExitRecord updated successfully.",
        },
    }



async def delete_exitrecord(
    args: dict,
    db: AsyncSession,
    ecosystem_ids: list | None = None
) -> dict:
    """Delete ExitRecord by id."""
    id_str = args.get("id", "")
    if not id_str:
        return {"success": False, "error": "id is required."}
    ...
    return {
        ...
        "data": {
            "id": str(id_uuid),
            "message": "ExitRecord deleted successfully.",
        },
    }
