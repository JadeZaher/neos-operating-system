...
**Acceptance Criteria:**
- AC-3.1: The skill defines the tagging taxonomy with required and optional tag categories. Required: domain (which domain produced the decision), layer (which NEOS layer), skill (which skill was used), precedent_level (routine, governance, constitutional), affected_parties (who was impacted). Optional: topic (free-text keyword tags), related_precedents (links to related decision records), ecosystem_scope (single AZPO, cross-AZPO, ecosystem-wide), urgency_at_time (normal, elevated, emergency).
