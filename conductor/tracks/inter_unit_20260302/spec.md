...
This track builds **Layer V (Inter-Unit Coordination)** of the NEOS governance stack: the skill layer that governs how autonomous units (ETHOS) coordinate with each other without requiring centralized mediation or hub-and-spoke authority. The layer encompasses cross-ETHOS requests, shared resource stewardship, federation agreements, liaison relationships, and polycentric conflict navigation.

NEOS is designed for polycentric governance -- multiple overlapping decision centers with no apex authority. This layer is where that design principle becomes most structurally demanding. ETHOS must be able to cooperate, share resources, resolve boundary disputes, and form multilateral agreements while maintaining their internal autonomy.
...
Layers I through IV govern how things work within an ETHOS or within the ecosystem as a single entity. Layer V governs the relationships between autonomous units that each have their own agreements, authority structures, decision processes, and resource pools. The structural challenge is different: there is no shared authority that can compel compliance. Coordination must emerge from mutual agreement, not hierarchical mandate.

This is the hardest governance problem in any non-sovereign system. Without a central arbiter, cross-unit disputes can escalate indefinitely or result in fragmentation. Layer V must provide enough structure for productive coordination while respecting the autonomy that makes ETHOS meaningful.
...
**Use:** Mutual-signature cross-ETHOS agreements where both sides must consent through their own ACT process. Inter-ETHOS liaison roles that maintain ongoing coordination relationships. Interoperability standards at the SHUR layer (shared protocols that all ETHOS agree to). Graduated engagement tiers (observe, cooperate, federate, integrate). Intermediate collaborative forums for multi-ETHOS discussions.

**Avoid:** Hub-and-spoke coordination where one ETHOS or body routes all cross-unit communication. Silent defaults as consent (if an ETHOS does not respond to a coordination request, this is NOT consent). Jurisdiction ambiguity (unclear which ETHOS's rules apply in shared spaces). The millet failure mode of needing a central mediator when two ETHOS cannot agree -- instead, provide structural protocols for lateral resolution.

### OmniOne Context

OmniOne's multi-site structure (Bali, planned Costa Rica, Mexico, Brazil) creates natural inter-unit coordination needs between SHUR communities. Different SHURs may have different space agreements, different resource pools, and different circle structures, but they share the Universal Agreement Field and the NEOS coordination protocols. The TH, AE, and OSC bodies provide ecosystem-wide coordination surfaces, but these do not override individual SHUR or ETHOS autonomy.

---

## Functional Requirements

### FR-1: Cross-ETHOS Request (`cross-ethos-request`)

**Description:** Enable any authorized participant to initiate and track a request that crosses ETHOS boundaries -- a request from one autonomous unit to another. This includes resource requests, information requests, collaboration proposals, and service requests. The skill defines how requests are routed, how the receiving ETHOS processes them through their own governance, and how outcomes are tracked across both units.

**Acceptance Criteria:**
- AC-1.1: The skill defines all required inputs (requester identity, originating ETHOS, target ETHOS, request type, request content, rationale, desired timeline, authority basis for the request).
- AC-1.2: The step-by-step process routes the request through the originating ETHOS's outbound authorization and the target ETHOS's inbound processing, with each ETHOS using its own ACT process.
- AC-1.3: The output artifact is a cross-ETHOS request record visible to both units, with status tracking and response documentation.
- AC-1.4: The authority boundary check ensures the requester has authorization from their own ETHOS to make the request and that neither ETHOS can unilaterally impose obligations on the other.
- AC-1.5: The capture resistance check addresses scenarios where a larger or wealthier ETHOS pressures a smaller one through volume or implied leverage.
...
- AC-2.2: The step-by-step process covers shared resource establishment (mutual agreement through each ETHOS's ACT process), stewardship appointment (rotating or shared), access rules (equitable across contributing ETHOS), reporting, and sunset.
- AC-2.3: The output artifact is a shared resource governance agreement signed through each participating ETHOS's consent process.
- AC-2.4: The authority boundary check prevents any single ETHOS from claiming exclusive control over shared resources and prevents stewards from favoring their home ETHOS.
- AC-2.5: The capture resistance check addresses larger ETHOS claiming proportional control based on larger contribution, and steward capture by their home unit.
...
**Description:** Draft, negotiate, and ratify bilateral or multilateral agreements between ETHOS. Federation agreements define the terms of ongoing coordination -- what each ETHOS commits to, what shared protocols apply, how disputes are handled, and how the agreement evolves. Every federation agreement goes through each participating ETHOS's own ACT process for ratification.
...
- AC-3.2: The step-by-step process covers negotiation (parallel advice phases in each ETHOS), drafting (collaborative or sequential), ratification (each ETHOS runs its own consent round), registration (in each ETHOS's agreement registry plus a shared registry if one exists), and review.
- AC-3.3: The output artifact is a federation agreement registered in all participating ETHOS' registries with mutual ratification records.
- AC-3.4: The authority boundary check ensures no ETHOS's negotiators exceed their mandate (negotiation authority must be explicitly scoped by the sending ETHOS).
- AC-3.5: The capture resistance check addresses power asymmetry (wealthier or larger ETHOS dictating terms), urgency pressure (rushing ratification), and precedent capture (early agreements becoming de facto standards that constrain later-joining ETHOS).
...
- AC-4.2: The step-by-step process covers liaison appointment (through authority-assignment in each ETHOS), mandate definition (what the liaison can communicate, explore, and recommend vs. what requires ETHOS-level consent), reporting cadence, and role review.
...
- AC-4.4: The authority boundary check prevents liaisons from making binding commitments on behalf of their ETHOS without explicit authorization and prevents the liaison role from becoming a bottleneck (others can still communicate across ETHOS boundaries).
...
**Description:** Resolve disputes where two or more ETHOS' authority claims conflict, their agreements contradict, or their resource decisions affect each other adversely. This is distinct from Layer VI (Conflict and Repair) which handles interpersonal and intra-ETHOS conflict. Polycentric conflict navigation handles structural conflicts between autonomous units that each have legitimate governance authority within their domains.
...
- AC-5.2: The step-by-step process provides a three-tier resolution approach: direct negotiation between affected ETHOS, facilitated dialogue with a mutually agreed neutral party (from a third ETHOS or the liaison pool), and structural resolution through federation agreement amendment.
...
- AC-5.4: The authority boundary check ensures no ETHOS or body can impose a resolution on another -- all resolutions require mutual consent through each ETHOS's own ACT process.
- AC-5.5: The capture resistance check addresses larger ETHOS leveraging their size to pressure resolution outcomes, neutral parties developing structural bias toward one ETHOS, and resolution fatigue leading to capitulation rather than genuine consent.
...
No skill in this layer may create, imply, or require a central authority that resolves inter-unit disputes by fiat. All resolution is lateral -- between the affected ETHOS themselves, optionally with facilitation but never with imposed outcomes. This is the millet system lesson: shared protocols without a Sultan.

### NFR-2: Dual-Consent Requirement

Every cross-ETHOS action requires consent from both (or all) participating ETHOS through their own internal ACT processes. No ETHOS can be bound by another's decision. Silence is not consent.
...
Inter-ETHOS relationships must support multiple engagement tiers. Not every ETHOS needs to be tightly federated with every other. Skills must accommodate relationships ranging from minimal (mutual acknowledgment) to deep (shared governance structures).
...
**I want** a clear process for making a formal request across ETHOS boundaries,
...
**When** they follow the cross-ethos-request skill process,
...
**As** an AI agent assisting a participant with a cross-ETHOS request,
**I want** to understand the dual-consent routing and which ETHOS's process applies at each stage,
**So that** I can guide the participant through the correct steps on both sides.

**Given** a cross-ETHOS request has been initiated,
**When** the AI agent reads the cross-ethos-request skill,
**Then** it can identify the originating ETHOS's outbound process, the target ETHOS's inbound process, and the current status.
...
neos-core/
  layer-05-inter-unit/
    ...
    cross-ethos-request/
      SKILL.md
      assets/
        cross-ethos-request-template.yaml
      references/
      scripts/
...
1. `cross-ethos-request` -- Anchor skill, defines the basic cross-unit interaction shape
...
- `act-consent-phase` -- Cross-ETHOS decisions use consent
...
- `resource-request` -- Cross-ETHOS resource requests reference the request pattern
...
- **Inter-ecosystem coordination** -- This layer handles ETHOS-to-ETHOS coordination within a single NEOS ecosystem. Coordination between two separate ecosystems both running NEOS is a future extension.
...
1. **Default engagement tier for new ETHOS**: When a new ETHOS forms within the ecosystem, what is its default relationship with existing ETHOS? Recommendation: default to "mutual acknowledgment" (observe tier) with active opt-in required for deeper engagement.

2. **Federation agreement registry location**: Where does a cross-ETHOS agreement live? In each ETHOS's registry? In a shared registry? Both? Recommendation: registered in each participating ETHOS's registry (federated storage) with a shared index maintained by the liaison coordination body (if one exists).

3. **Neutral party selection for conflict navigation**: How is a neutral facilitator selected when two ETHOS cannot agree on one? Recommendation: each ETHOS proposes three candidates from other ETHOS, overlapping names are selected, if no overlap both lists are combined and random selection is used.
